...
export class CmsHomeRecommendations {
  ...
  public toggleSong(songId: number): void {
    this.selectedSongIds.update((ids) => this.toggleId(ids, songId));
  }

  public toggleArtist(artistId: number): void {
    this.selectedArtistIds.update((ids) => this.toggleId(ids, artistId));
  }
  ...
  public async save(): Promise<void> {
    ...
    try {
      ...
      await this.loadData();
      this.message.set('首頁推薦已更新');
    } catch (err) {
      console.error('CMS 更新首頁推薦失敗：', err);
      this.message.set('更新失敗，請確認 API server 已重啟後再試');
    } finally {
  ...
  private async loadData(): Promise<void> {
    ...
    try {
      const [songs, artists, recommendations] = await Promise.all([
      ...
      this.message.set(recommendation ? '' : '尚未建立首頁推薦，請勾選內容後儲存。');
    } catch (err) {
